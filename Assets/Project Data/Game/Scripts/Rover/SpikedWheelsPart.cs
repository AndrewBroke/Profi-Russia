...
namespace Watermelon
{
    public class SpikedWheelsPart : WheelPartBehavior
    {
        ...
        public override void DriveUpdate()
        {
            ...
            if (IsPressed && Physics2D.OverlapCircle(transform.position, circleCollider.radius * 1.6f, PhysicsHelper.GetLayerMask(PhysicsLayer.Ground)))
            {
                Master.Rigidbody.AddForceAtPosition(-transform.up * (10 + Master.Rigidbody.velocity.sqrMagnitude * 0.5f), transform.position);
            }
        }
