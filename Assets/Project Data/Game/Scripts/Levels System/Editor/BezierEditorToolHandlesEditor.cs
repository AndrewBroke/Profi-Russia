...
public class BezierEditorToolHandlesEditor : Editor
{
    ...
    static Vector3[] batteryBodyPoligon;
    static Vector3[] batteryCapPoligon;
    ...
    private void DrawHandles()
    {
        ...
        for (int i = 0; i < BezierEditorTool.Instance.Groups.Count; i++)
        {
            ...
            for (int j = 0; j < BezierEditorTool.Instance.Groups[i].slices.Count; j++)
            {
                ...
                {
                    var pos = Handles.PositionHandle(BezierEditorTool.Instance.Groups[i].position, Quaternion.identity);
                    ...
                    Handles.Label(BezierEditorTool.Instance.Groups[i].position, "Group Position");
                    Handles.Label(BezierEditorTool.Instance.Groups[i].position + Vector3.down * 0.25f, BezierEditorTool.Instance.Groups[i].position.ToString());
                    groupPosition = BezierEditorTool.Instance.Groups[i].position;

                    if (BezierEditorTool.Instance.SelectedBezierSliceIndex == j)
                    {
                        tempColor = BezierEditorTool.Instance.SelectedSliceColor;
                    ...
                    {
                        tempColor = BezierEditorTool.Instance.SelectedBezierColor;
                    }
                }
                else
                {
                    tempColor = BezierEditorTool.Instance.UnselectedBezierColor;
                }
            ...
            DrawBezierGroup(tempColor, groupPosition, i);
        }

        Handles.color = backupColor;
    }

    public static void DrawBezierGroup(Color sliceColor, Vector3 groupPosition, int groupIndex)
    {
